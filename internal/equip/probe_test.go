...
func TestProbeMeasuresALegacyServerOnEveryToolsPage(t *testing.T) {
	...
	if got := row(t, session.View(), "fake"); got.CostUnknown || got.Cost != 11 {
...
func TestProbeMeasuresAModernServerThroughServerDiscover(t *testing.T) {
	...
	if got := row(t, session.View(), "fake"); got.CostUnknown || got.Cost != 7 {
...
// probed probes the MCP server fake in a session of repo and fails the test
// on error.
func probed(t *testing.T, machine *equiptest.Machine, repo string) {
	t.Helper()
...
// probedRow probes the MCP server fake in a session of repo and returns its
// row.
...
func TestProbeStartsAServerAsCodexDoesWhenOnlyCodexHasItOn(t *testing.T) {
	...
	writeFile(t, filepath.Join(machine.CodexHome, "config.toml"),
...
func TestCodexServerStaysMeasuredWhenItsStateChanges(t *testing.T) {
	...
	repo := machine.Repo("app")
	exe, _ := fakeConfig(t, "modern", "")["command"].(string)
	config := "[mcp_servers.fake]\ncommand = " + strconv.Quote(exe) + "\nenv = { " + fakeMCPEnv + " = \"modern\" }\n"
	writeFile(t, filepath.Join(machine.CodexHome, "config.toml"), config)
	probed(t, machine, repo)

	writeFile(t, filepath.Join(machine.CodexHome, "config.toml"), config+"enabled = true\n")
...
func TestMeasuredPluginServerAddsItsCostToThePlugin(t *testing.T) {
	...
	if got := row(t, session.View(), "github@official"); got.Cost != 7 {
...
func TestCodexCostsAServersNameAndInstructions(t *testing.T) {
	...
	// fake and "Use fake.": 13 bytes.
	if got := session.Detail("mcp:fake").Costs[equip.Codex]; got != 4 {
		t.Errorf("Codex cost = %d, want 4", got)
	}
}
...
// header X-Key: k. It rejects server/discover, answers initialize with a
// session, and once initialized lists the tool a as an event stream.
func fakeHTTPServer(t *testing.T) string {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, httpReq *http.Request) {
		...
		switch {
		case httpReq.Header.Get("X-Key") != "k":
			writer.WriteHeader(http.StatusUnauthorized)
		case req.Method == "initialize":
			writer.Header().Set("Content-Type", "application/json")
			writer.Header().Set("Mcp-Session-Id", "s1")
			_, _ = fmt.Fprintf(writer, `{"jsonrpc":"2.0","id":%s,"result":{"instructions":"Use fake."}}`, req.ID)
